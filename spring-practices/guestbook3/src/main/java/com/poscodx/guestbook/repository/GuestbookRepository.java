...
public class GuestbookRepository {
	...
	public int insert(GuestbookVo vo) {
		return jdbcContext.update("insert into guestbook values(null, ?, ?, ?, ?)",
				new Object[] { vo.getName(), vo.getPassword(), vo.getContent(), new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()) });

	}

	public List<GuestbookVo> findAll() {
		return jdbcContext.query("select no, name, reg_date, content from guestbook order by reg_date desc",
