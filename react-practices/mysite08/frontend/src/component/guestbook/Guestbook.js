...
export default function Guestbook() {
  ...
  const fetchMessages = async function () {
    ...
    const startNo =
      messagesInState.length === 0
        ? 0
        : messagesInState[messagesInState.length - 1].no;
    try {
    ...
    <>
      ...
      >
        ...
        <div>
            ...
            onSubmit={async (e) => {
              try {
                const response = await fetch(
                  `/api/guestbook/${e.target.no.value}`,
                  {
                    method: "delete",
                    headers: {
                ...
                setMessages(
                  messages.filter((message) => message.no != json.data)
                );
              } catch (err) {
                console.error(err);
              }
          ...
          >
            ...
            <input
              ...
              onChange={(e) =>
                setModalData(
                  Object.assign({}, modalData, { password: e.target.value })
                )
              }
              autoFocus={true}
            />
        ...
        <div className={modalStyles["modal-dialog-buttons"]}>
          <button
            onClick={() =>
              refDeleteForm.current.dispatchEvent(
